...
def lowerbound(m_gx, m_xm, m_mx, m_xg,phi_m, phi_g):
	...
	for n in xrange(N):
		LB += np.dot(np.array([m_gx[0]*m_mx[0], -m_gx[0]/2.]).T, np.array([xn[n], xn[n]**2]))
	LB += N/2.*(m_gx[1]-m_gx[0]*m_mx[1]-np.log(2*math.pi))

	LB += np.dot((phi_mu - phi_m).T, m_mx) + 1./2*(np.log(beta) - beta*m**2 + beta_mu*m_mu**2 - np.log(beta_mu))
	LB += np.dot((phi_gamma - phi_g).T, m_gx) + a*np.log(b) - gammaln(a) - a_gamma*np.log(b_gamma) + gammaln(a_gamma)
...
while ((it<It) & ((it<2) | (inc>1e-10))):
	...
	for n in xrange(N):
		m_xg[n,:] = np.array([-1./2*(xn[n]**2-2*xn[n]*m_mx[0]+m_mx[1]),1./2])
