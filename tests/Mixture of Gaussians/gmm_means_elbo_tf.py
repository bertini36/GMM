...
ELBO = tf_dirichlet_expectation(lambda_pi_res)
ELBO = tf.add(ELBO, tf.matmul(tf.sub(alpha, lambda_pi), tf_dirichlet_expectation(lambda_pi_res)))
ELBO = tf.add(ELBO, tf.mul(K/2., tf.log(tf.matrix_determinant(tf.mul(beta_o, Delta_o)))))
...
for k in range(K):
	...
	a4 = tf.div(tf.mul(tf.cast(D, tf.float64), beta_o), tf.mul(2., lambda_mu_beta_res[k]))
	a5 = tf.mul(1/2., tf.log(tf.mul(tf.pow(lambda_mu_beta_res[k], 2), tf.matrix_determinant(Delta_o))))
