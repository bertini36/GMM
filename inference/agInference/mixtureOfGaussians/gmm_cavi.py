...
def dirichlet_expectation(alpha, k):
    ...
    """
    return agscipy.psi(agnp.add(alpha[k], agnp.finfo(agnp.float32).eps)) \
...
def elbo((lambda_phi, lambda_pi, lambda_m, lambda_w, lambda_beta, lambda_nu)):
    ...
    for k in range(K):
        ...
        elbop = agnp.add(elbop, (alpha_o[k] - 1 + agnp.sum(
        ...
        ss_niw = [
            ...
            (-D / 2.) * (1 / lambda_beta[k]) - (1 / 2.) * lambda_nu[
                ...
                lambda_m[k, :]),
            agnp.add((D / 2.) * agnp.log(2.), (1 / 2.) * agnp.subtract(agnp.sum(
        ...
        agnp.add(elboq, (lambda_nu[k] + D + 2) * ss_niw[3])
