...
parser.add_argument('-dataset', metavar='dataset', type=str,
                    default='../../../data/synthetic/k8/data_k8_1000.pkl')
...
def update_lambda_w(lambda_w, lambda_phi, lambda_beta,
                    lambda_m, w_o, beta_o, m_o, xn_xnt, xn, Nk, K, N, D):
    """
    Update lambda_w
...
def NIW_sufficient_statistics(k, D, lambda_nu, lambda_w, lambda_m, lambda_beta):
    ...
    return np.array([
        ...
        (D / 2.) * np.log(2.) + (1 / 2.) * np.sum(
            [psi(((lambda_nu[k] / 2.) + ((1 - i) / 2.))) for i in range(D)]) - ( # this was wrong, the Psi function is for each summand
...
def NIW_nat_params(k, D, lambda_m, lambda_w, lambda_beta, lambda_nu):
    return np.array([lambda_beta[k]*lambda_m[k,:],
                     (lambda_w[k,:,:]+lambda_beta[k]*np.outer(lambda_m[k,:],lambda_m[k,:])),
                     lambda_beta[k],
                     lambda_nu[k]+D+2
                     ])
...
def elbo(lambda_phi, lambda_pi, lambda_m, lambda_w, lambda_beta, lambda_nu,
    ...
    elbo  = K * gammaln(np.sum(alpha_o)) - np.sum(gammaln(alpha_o)) - K * gammaln(np.sum(lambda_pi)) + np.sum(gammaln(lambda_pi))
...
def main():
    ...
    lbs = []
    n_iters = 0
    #print lambda_phi.shape
    #print lambda_pi.shape
    for _ in range(MAX_ITERS):
        ...
        lambda_w = update_lambda_w(lambda_w, lambda_phi, lambda_beta,
                                   lambda_m, w_o, beta_o, m_o, xn_xnt, xn, Nks, K, N, D)
    ...
    if VERBOSE:
        ...
        for k in range(K):
            ...
            print('SD k{}: {}'.format(k, np.sqrt(np.diag(lambda_w[k, :, :] / (lambda_nu[k] - D - 1)))))
        final_time = time()
