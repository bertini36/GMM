...
def ag_dirichlet_expectation(alpha):
    if len(alpha.shape) == 1:
        return agscipy.psi(alpha + agnp.finfo(np.float32).eps) - agscipy.psi(
            agnp.sum(alpha))
...
def ag_log_beta_function(x):
    return agnp.sum(
        agscipy.gammaln(x + agnp.finfo(np.float32).eps)) - agscipy.gammaln(
        agnp.sum(x + agnp.finfo(np.float32).eps))


def ELBO((lambda_pi, phi, lambda_mu_m, lambda_mu_beta)):
    lambda_pi_aux = np.log(1 + np.exp(lambda_pi) + machineprecion)
    phi_aux = (np.exp(phi) + machineprecion) / np.tile(
        (np.exp(phi) + machineprecion).sum(axis=1), (K, 1)).T
    ...
    ELBO = ag_log_beta_function(lambda_pi_aux) - ag_log_beta_function(alpha) \
           + np.dot(alpha - lambda_pi_aux,
                    ag_dirichlet_expectation(lambda_pi_aux)) \
           + K / 2. * np.log(np.linalg.det(beta_o * Delta_o)) + K * D / 2.

    for k in xrange(K):
        ELBO -= beta_o / 2. * np.dot((lambda_mu_m[k, :] - m_o),
                                     np.dot(Delta_o,
                                            (lambda_mu_m[k, :] - m_o).T))
        for n in xrange(N):
            ELBO += phi_aux[n, k] * (
                ag_dirichlet_expectation(lambda_pi_aux)[k] - np.log(
                    phi_aux[n, k]) + 1 / 2. * np.log(1. / (2. * math.pi))
                - 1 / 2. * np.dot((xn[n, :] - lambda_mu_m[k, :]),
                                  np.dot(Delta_o,
                                         (xn[n, :] - lambda_mu_m[k, :]).T))
...
def ELBO1((lambda_pi, phi)):
    ...
    for k in xrange(K):
        ...
        for n in xrange(N):
            ELBO += phi_aux[n, k] * (
                ag_dirichlet_expectation(lambda_pi_aux)[k] - np.log(
                    phi_aux[n, k]) + 1 / 2. * np.log(
                    np.linalg.det(Delta_o) / (2. * math.pi)) \
                - 1 / 2. * np.dot((xn[n, :] - lambda_mu_m[k, :]),
                                  np.dot(Delta_o,
                                         (xn[n, :] - lambda_mu_m[k, :]).T))
                - D / (2. * lambda_mu_beta_aux[k]))
...
if __name__ == "__main__":
    ...
    parser.add_argument('-m_o', metavar='m_o', nargs='+', type=float,
                        default=[0., 0.])
    parser.add_argument('-beta_o', metavar='beta_o', nargs='+', type=float,
                        default=0.01)
    ...
    for i in xrange(args.maxIter):
        ...
        print  "It: ", i, "ELBO: ", ELBO(
            (lambda_pi, phi, lambda_mu_m, lambda_mu_beta))

    aux = (np.exp(phi) + machineprecion) / np.tile(
        (np.exp(phi) + machineprecion).sum(axis=1), (K, 1)).T
    plt.scatter(xn[:, 0], xn[:, 1], c=np.array(
        1 * [np.random.choice(K, 1, p=aux[n, :])[0] for n in xrange(N)]))
    plt.show()
