...
def initialize(xn, K, alpha, m_o, beta_o, nu_o, Delta_o):
	...
	for k in xrange(K):
		S = 1./Nk[k]*np.dot((xn-xk_[k,:]).T,np.dot(np.diag(phi[:,k]),(xn-xk_[k,:])))
		lambda_delta_W[k, :, :] = np.linalg.inv(np.linalg.inv(Delta_o) + Nk[k]*S \
			+ beta_o*Nk[k]/(beta_o + Nk[k]) * np.dot(np.tile((xk_[k,:] - m_o),(1,1)).T,np.tile(xk_[k,:]-m_o,(1,1))))

	return lambda_pi, phi, lambda_mu_m, lambda_mu_beta, lambda_delta_nu, lambda_delta_W
...
if __name__ == "__main__":
	...
	lambda_pi, phi, lambda_mu_m, lambda_mu_beta, lambda_delta_nu, lambda_delta_W = initialize(xn, K, alpha, m_o, beta_o, nu_o, Delta_o)
	...
	for it in xrange(args.maxIter):
		...
		for n in xrange(N):
			aux = np.copy(Elogpi)
			for k in xrange(K):
				aux[k] += 1./2*(psi(lambda_delta_nu[k]/2.)+psi((lambda_delta_nu[k]-1.)/2.) \
					+ np.log(np.linalg.det(lambda_delta_W[k,:,:]))-2./lambda_mu_beta[k]	
					- lambda_delta_nu[k]*(np.dot((xn[n,:]-lambda_mu_m[k,:]).T, np.dot(lambda_delta_W[k,:,:],(xn[n,:]-lambda_mu_m[k,:])))))
