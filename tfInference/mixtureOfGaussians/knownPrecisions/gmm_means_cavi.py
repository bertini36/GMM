...
for n in xrange(N):
    k_list = []
    for k in xrange(K):
        c2 = tf.reshape(tf.subtract(xn_tf[n, :], lambda_mu_m[k, :]), [1, D])
        c3 = tf.matmul(Delta_o, tf.reshape(
            tf.transpose(tf.subtract(xn_tf[n, :], lambda_mu_m[k, :])), [D, 1]))
    ...
    phi_tmp.append(tf.reshape(tf_exp_normalize(tf.stack(k_list)), [K, 1]))
assign_phi = phi.assign(tf.reshape(tf.stack(phi_tmp), [N, K]))
...
d1 = tf.transpose(
    tf.reshape(tf.tile(tf.div(tf.cast(1., tf.float64), lambda_mu_beta), [D]),
               [D, K]))
d2 = tf.add(tf.multiply(m_o, beta_o), tf.matmul(tf.transpose(phi), xn_tf))
assign_lambda_mu_m = lambda_mu_m.assign(tf.multiply(d1, d2))
