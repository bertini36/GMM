...
aux1 = np.tile(1./lambda_mu_beta_aux, (2, 1)).T
aux2 = (beta_o_aux * m_o_aux + np.dot(phi_aux.T, data['xn']))
...
ELBO = tf.add(ELBO, 
			  tf_dot(tf.sub(alpha, lambda_pi), tf_dirichlet_expectation(lambda_pi)))
ELBO = tf.add(ELBO,  
...
for k in xrange(K):
	...
	ELBO = tf.sub(ELBO,
				  tf.mul(tf.div(beta_o, 2.), tf_dot(tf.sub(lambda_mu_m[k,:], m_o), 
				  tf_dot(Delta_o, tf.transpose(tf.sub(lambda_mu_m[k,:], m_o))))))
	ELBO = tf.sub(ELBO,
	...
	for n in xrange(N):
		...
		aux2 = tf.mul(1/2.,
					  tf_dot(tf.sub(xn[n,:], lambda_mu_m[k,:]), 
					 		 tf_dot(Delta_o, tf.transpose(tf.sub((xn[n,:], lambda_mu_m[k,:]))))))
		ELBO = tf.add(ELBO,
					  tf.mul(phi[n,k],
